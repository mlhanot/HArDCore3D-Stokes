...
// Copied from GMpoly_cell
/// Generic template to compute the Gram Matrix of a pair of gradient bases
template<typename BasisType1, typename BasisType2>
Eigen::MatrixXd GramMatrix(
                    const Face & F,                             ///< Face to which the basis corresponds
                    const GradientBasis<BasisType1> & grad_basis1,       ///< First basis
                    const GradientBasis<BasisType2> & grad_basis2,       ///< Second basis
                    MonomialFaceIntegralsType mono_int_map = {} ///< Optional list of integrals of monomials up to the sum of max degree of basis1 and basis2
                    )
  {
    ...
    for (size_t m=0; m<2; m++){
      gm += GMScalarDerivative(F, grad_basis1.ancestor(), grad_basis2.ancestor(), m, m, intmap);
    }
